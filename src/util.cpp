...
int nAnonymizeXaruCoinAmount = 500;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\XaruCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\XaruCoin
    // Mac: ~/Library/Application Support/XaruCoin
    // Unix: ~/.XaruCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XaruCoin";
#else
    fs::path pathRet;
...
void createConf()       //Automatic XaruCoin.conf generation
{
    srand(time(NULL));
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "XaruCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "xarucoind.pid"));
