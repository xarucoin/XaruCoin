...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("XaruCoin"))
        return false;
...
{
    // Convert XaruCoin:// to XaruCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("XaruCoin://"))
    {
        uri.replace(0, 12, "XaruCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "XaruCoin.lnk";
}
...
{
    return GetAutostartDir() / "XaruCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("XaruCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  XaruCoin-qt [" + tr("command-line options") + "]                     " + "\n";
